...
#define DEFAULT_PUBLISH_TOPIC     "projeto/sensor_status"
#define DEFAULT_SUB_TOPIC         "refresh/topic"
...
#define DEFAULT_SUBSCRIBE_CMD_TYPE   "topic"
            ...
            uint16_t chunk_len)
{
  ...
  printf("\nn= %u",n);
  conf.pub_interval = n*CLOCK_SECOND;
  return;
...
{
  int len = snprintf(sub_topic, BUFFER_SIZE, DEFAULT_SUB_TOPIC);
  if(len < 0 || len >= BUFFER_SIZE) {
...
publish(void)
{
  ...
  float sensor_volt,sensor, RS_gas,ratio,CO;
  seq_nr_value++;
  buf_ptr = app_buffer;
...
*/
 ...
 // BUSCAR TEMPs
  ...
  {
    len = snprintf(buf_ptr, remaining, ",\"Temp\":\"%u.%u\"", temperature / 10, temperature % 10);
  }
  ...
  buf_ptr += len;
  //Buscar CO ppt:
  sensor = adc_zoul.value(ZOUL_SENSORS_ADC3);
  sensor_volt = sensor/1000;
  RS_gas = (5-sensor_volt)/sensor_volt;
  ratio = RS_gas/1;

  if(ratio > 1)
  {
    CO = (1.85-ratio)/0.0015;
  }
  else if(ratio > 0.85 && ratio <= 1)
  {
    CO = (1.52-ratio)/0.00083;
  }
  else
  {
    CO = (1.13-ratio)/0.00035;
  }
  if (CO<0)
  {
    CO = 0;
  ...
  len = snprintf(buf_ptr, remaining, ",\"CO\":\"%u\"", (int)CO);
  remaining -= len;
  buf_ptr += len;
